...
const int samples = 40;// The number of "copies" of the canvas made to emulate the "smoke" effect
const float decay = 1.1;// Decay of the light in each sample
...
void fragment(vec2 uv, vec2 pos, inout vec4 color) {
    ...
    float reflectiveness = pow(texColor.r*1.6, 10.0) * 0.5;
    vec3 col = vec3(0);
    if (uv.x > 0.5) {
        vec2 lightSource = vec2(0.3  , uv.y);
        ...
        for (int i=0; i<samples; i++) {
            ...
            illumination_decay *= 0.95;
        }
        color = vec4(col * reflectiveness, texColor.a);
    } else {
        vec2 lightSource = vec2(0.7, uv.y);
        vec2 _uv = uv;
        vec2 distance = (_uv - lightSource) * (1. / float(samples));
        float illumination_decay = 1;
        for (int i=0; i<samples; i++) {
